...
void main() {
	...
	float rot_step = radians( 360.0 / 5.0 );
	
	for( float i =0; i < 5; i++ ) {
	
		mat3 rot = rotationMatrix( up, rot_step * i );
		mat3 rot2 = rotationMatrix( cross( tip-base, up-tip ), -0.7 );
		...
		for( float s =-1.0; s <= 1.0; s += 2.0 ) {
			...
			for( float t =0; t < 1.0; t += 0.2 ) {
				vec3 v1 = rot* rot2 * (bezier2( t, base, base, tip ) - base ) + base;
				vec3 v2 = rot* rot2 * (bezier2( t, base, curve, tip ) - base ) + base;
			...
			emitAndMult( rot * rot2 * (tip-base) + base, n, t1 );
		}
	}
}